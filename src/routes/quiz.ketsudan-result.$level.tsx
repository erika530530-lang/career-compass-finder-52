...
import { ketsudanBand, ketsudanOgImage } from "@/lib/quizzes/ketsudan-og";

export const Route = createFileRoute("/quiz/ketsudan-result/$level")({
  loader: ({ params }) => {
    const band = ketsudanBand(params.level);
  ...
  head: ({ params, loaderData }) => {
    ...
    const title = `13問でわかる！あなたの決断力タイプは「${loaderData.title}」｜ピクセルポップ 決断力診断`;
    const description = `決断力タイプは「${loaderData.title}」。コンビニで5分悩む人も即決する人も、13問で「決める力」を採点できる無料診断です。`;
    const url = canonical(`/quiz/ketsudan-result/${params.level}`);
    const image = ketsudanOgImage(params.level);
    return {
      meta: [
        { title },
  ...
  component: KetsudanResultPage,
});

function KetsudanResultPage() {
  const data = Route.useLoaderData();
  const recos = quizzes.filter((q) => q.id !== "ketsudan").slice(0, 4);
      ...
      <div className="mx-auto w-full max-w-md px-4 pb-24 pt-4 md:max-w-2xl md:px-6">
        ...
        <section className="animate-pop card-surface mt-5 overflow-hidden">
          ...
          <div className="p-5">
            ...
            <ShareRow
              quizId="ketsudan"
              text={`私の決断力タイプは「${data.title}」でした${data.emoji}`}
            />
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          {recos.map((r) => (
            <QuizRow key={r.id} quiz={r} fromQuizId="ketsudan" location="ketsudan_result" />
          ))}
        </div>
